...
uint graph::cmf_backtracking(vector<node_id>& clique) const{
    ...
    if(this->_quant_edges == this->_quant_nodes*(this->_quant_nodes-1)>>1){
        ...
        */
        ...
        bool inicialization_req;
        #pragma omp parallel for schedule(static) default(none) collapse(1)\
            firstprivate(candidates,partial_solution)\
            private(bound_joinable_nodes,bound_best_frontier,inicialization_req,partial_frontier)\
            shared(max_frontier,clique,cout)
        //#ifdef _OPENMP
        ...
        //#endif//_OPENMP
            ...
            while(
                    ...
                    //!candidates[0].empty() &&
                    //#endif//_OPENMP
                    !partial_solution.empty())
            {
                if(
                    //#ifdef _OPENMP
                    inicialization_req ||
                    //#endif//_OPENMP
                    !candidates[partial_solution.size()].empty())
                {
                    if(inicialization_req){
                        inicialization_req = false;
                        ...
                        ++rit)
                    {
                        if(
                            ...
                            *rit > i &&
                            //#endif//_OPENMP
                            this->_nodes[*rit-1]->_degree > partial_solution.size()<<1)
                        {
