...
\begin{pseudocodigo}[Algoritmo Exacto para \emph{CMF} - Complejidad]
    ...
    \Else
        ...
        \EndWhile \Compl{Blue}{Costo \emph{mientras}: }{$2^{\sfrac{n}{2}}$}{veces $\times$ $\mathcal O(n) = \mathcal O(n\cdot 2^{\sfrac{n}{2}})$\label{costo_ciclo} }
...
\par Como ya sabemos que el tama\~no de la \emph{CMF} estar\'a acotado por
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        \draw[<->] (aux2) -- node[fill=white] {$\displaystyle\frac{n}{2}$} (aux2|-theta.184);

        % the brace at the bottom
        \draw[decorate,decoration={brace,raise=2pt}] (theta.south east) -- node[label={[label distance=5pt]below:$2^{\frac{n}{2}} \times \mathcal O(n)$}]{}  (theta.south west);

        % the rule with the total
        \node[below = 7cm of t2.east,anchor=east] {Complejidad Total del Ciclo: $\mathcal O\Biggl(\displaystyle\sum_{k=0}^{\sfrac{n}{2}} 2^k \cdot n\Biggl) = \mathcal O\Biggl(n \cdot \sum_{k=0}^{\sfrac{n}{2}} 2^k\Biggl) = \mathcal O\Bigl(n \cdot 2^{\sfrac{n}{2}}\Bigl)$};
    \end{tikzpicture}
\end{figure}
\bigskip
...
\par As\'i pues, llegamos a la conclusi\'on de que la complejidad asint\'otica
    temporal de nuestro algoritmo exacto es $\mathcal O(n \cdot 2^{\sfrac{n}{2}})$.
