...
\par Entonces, $\delta(K)$, es expresable como:
\begin{equation*}
    \displaystyle\sum_{v \in K}d(v) - (|K|-1)
\end{equation*}

\par Como la cantidad de nodos en $K$ es justamente $|K|$, la expresi\'on
    que nos queda es:
\begin{equation*}
    \delta(K) = -|K|(|K|-1)\displaystyle\sum_{v \in K}d(v)
\end{equation*}
...
\begin{equation*}
    \delta(K+v) = \delta(K) + \underbrace{d(v)-|K|}_{\text{Lo que se gana}}
...
\subsection*{Sobre los generadores de instancias aleatorias\label{notas:datasets}}
...
\subsection*{Sobre la experimentaci\'on\label{notas:experimentacion}}
